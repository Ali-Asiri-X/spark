...
/**
 * Utility for executing different types of filters.
 */
final class FilterExecutor {

    /**
     * Executes filters matching an HTTP request for a given HTTP method type.
     *
     * @param context    the route context
     * @param httpMethod the HTTP method to find filters for
     * @throws Exception if an exception occurs during execution
     */
    static void execute(RouteContext context, HttpMethod httpMethod) throws Exception {
        Object content = context.body().get();

        List<RouteMatch> matchSet = context.routeMatcher().findMultiple(httpMethod,
                context.uri(),
                context.acceptType());
